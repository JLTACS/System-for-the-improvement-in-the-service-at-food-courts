 ...
 * MFRC522_NFC.h
 *
 * Created: 25/03/2016 09:34:55 p. m.
 *  Author: joseluis
 */ 


#ifndef MFRC522_NFC_H_
#define MFRC522_NFC_H_
...
unsigned char read[MAX_LEN];

//------------------------------------------------------------------------------


/* Escribir a bloque de la tarjeta */
void writeTagBlockMemory(uchar *data4){
	uchar serNum[5];
	//clave por default de la tarjeta 
	uchar sectorX_KeyA[]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; 
	uchar i,j,ij;
	uchar status;
    uchar str[MAX_LEN];
	for(ij = 0; ij < 10; ij++){
		//Buscar y activar tarjeta
		status = MFRC522_Request(PICC_REQIDL, str);	
		if (status == MI_OK){  }
		//Obtener numero de serie de la tarjeta
		status = MFRC522_Anticoll(str);
		for(i=0; i<5; i++){	 serNum[i]=str[i];  }	
		if (status == MI_OK){  }
		status = MFRC522_SelectTag(serNum);	

		status = MFRC522_Auth(0x60,4,sectorX_KeyA,serNum);	
		if(status==MI_OK){   
...
/* Leer dato en ASCII de la tarjeta	 */
void readDataASCII(uchar bloque){
	uchar serNum[5];
	uchar ij;
	uchar sectorX_KeyA[]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; 
...
for(ij = 0; ij < 10; ij++){
		...
		for(i=0; i<5; i++){	 serNum[i]=read[i];  }	
		if (status == MI_OK){   
                                                               }
		status = MFRC522_SelectTag(serNum);	
		status = MFRC522_Auth(0x60,bloque,sectorX_KeyA,serNum);	
...
/* Poner a uno bit/s de algun registro del MFRC522		*/
void SetBitMask(uchar reg, uchar mask)  {
    uchar tmp;
    tmp = Read_MFRC522(reg);
    Write_MFRC522(reg, tmp | mask);  	}	// set bit mask

/*Limpiar registros del MFRC522		*/
void ClearBitMask(uchar reg, uchar mask) {
    uchar tmp;
    tmp = Read_MFRC522(reg);
    Write_MFRC522(reg, tmp & (~mask));	 }  // clear bit mask

/* Encender antena del modulo		*/
void AntennaOn(void) {
	uchar temp;
	temp = Read_MFRC522(TxControlReg);
	if (!(temp & 0x03)){  SetBitMask(TxControlReg, 0x03);  } }

/* Apagar antena del modulo	*/
void AntennaOff(void){	
...
/* Comunicacion entre MFRC522 y tarjeta	*/
uchar MFRC522_ToCard(uchar command, uchar *sendData, uchar sendLen, uchar *backData, uint *backLen){
    uchar status = MI_ERR;
    uchar irqEn = 	0x00;
    uchar waitIRq = 0x00;
    ...
    uint i;
    switch (command) {
        case PCD_AUTHENT: 	{	//Verificar clave de tarjeta	
			irqEn = 0x12;
			waitIRq = 0x10;
			break;			}
		case PCD_TRANSCEIVE:{	//Enviar datos a FIFO
			irqEn = 0x77;
			waitIRq = 0x30;
			break;			}
		default:	break; 	}
    Write_MFRC522(CommIEnReg, irqEn|0x80);	//Activar interrupciones
    ClearBitMask(CommIrqReg, 0x80);			//Limpiar interruociones
    SetBitMask(FIFOLevelReg, 0x80);			//inicializar FIFO 
	Write_MFRC522(CommandReg, PCD_IDLE);	//Enviar a modo Idle
...
/* Leer numero de serie de la tarjeta y verificarlo*/
uchar MFRC522_Anticoll(uchar *serNum){
    uchar status;
    uchar i;
	uchar serNumCheck=0;
...
/* Seleccion de tarjeta y lectura de tamaño de tarjeta		 */
uchar MFRC522_SelectTag(uchar *serNum) {
    uchar i;
	uchar status;
	uchar size;
...
/* Escribir datos a bloque de la tarjeta		*/
uchar MFRC522_Write(uchar blockAddr, uchar *writeData) {
    uchar status;
    uint recvBits;
    uchar i;
